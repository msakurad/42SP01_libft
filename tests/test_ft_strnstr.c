...
void	test_ft_strnstr_littlefound_returnsfirstocurrence(void)
{
	...
	printf("\nbig=\"%s\" little=\"%s\" n=10\n", big, little);
	//TEST_IGNORE_MESSAGE("teste ft vs lib fail -> return 1 x 8");
	TEST_ASSERT_EQUAL_STRING(&big[3], ft_strnstr(big, little, 10));
}

void	test_ft_strnstr_littlesimilarbutnotequal_returnsnull(void)
{
	char	*big;
	char	*little;
...
void	run_test_ft_strnstr(void)
{
	...
	RUN_TEST(test_ft_strnstr_littlesimilarbutnotequal_returnsnull);
	RUN_TEST(test_ft_strnstr_littleempty_returnsbig);
	RUN_TEST(test_ft_strnstr_littleistotallydifferentnotfound_returnsnull);
	RUN_TEST(test_ft_strnstr_littleisinbigbutlenisshort_returnsnull);
